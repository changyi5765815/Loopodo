...
public class GroupCompanyProcessor extends BaseProcessor {

	public GroupCompanyProcessor(Module module, DataHandle dataHandle) {
	...
	public void makeView() throws Exception {
		...
		if ("list".equals(action)) {
			...
			setFormData("q_groupFlag", "1");
			initPageByQueryDataList("user", getFormDatas(), "datas", "", new Vector<String>(), "order by groupSortIndex, userID desc");
	...
	public void addCompanyUsersAction() throws Exception {
		...
		String sql = "update user set groupFlag = 1 where userID in (" + selectedValues + ")";
		DBProxy.update(getConnection(), "user", sql, new Vector<String>());
		
		setAjaxJavascript("closeInfoWindow();postModuleAndAction('groupCompany', 'list')");
	}
}
